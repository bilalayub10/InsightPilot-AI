...
- 🔄 **3-tier AI fallback** — Gemini → OpenRouter → deterministic pattern-based logic, so the app never fails even if both AI providers are down
...
**Provider chain:** Google Gemini (`gemini-2.0-flash`) is tried first. On failure, it falls back to OpenRouter (`google/gemini-2.5-flash`). If both fail, a deterministic, pattern-based fallback (trend direction, concentration %, distribution shape, etc., computed from the real data) takes over — the app degrades gracefully instead of breaking.
...
**AI:** Google Gemini (`gemini-2.0-flash`) as primary provider, OpenRouter (`google/gemini-2.5-flash`) as fallback, via the `google-genai` SDK
...
```
React (Vite frontend)  →  FastAPI backend  →  Gemini → OpenRouter → Deterministic fallback
```
